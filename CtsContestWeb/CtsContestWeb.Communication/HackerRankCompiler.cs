            ...
            if (compileResult.Compiled)
            {
                for (int i = 0; i < task.Outputs.Count; i++)
                {
                    ...
                    if (expected != actual)
                    {
                        ...
                        compileResult.Message = data.result.stderr[i].Value == false ? (data.result.message[i].Value.ToString() != "Success" ? data.result.message[i].Value.ToString() : "") : data.result.stderr[i].Value.ToString();
                        break; 
                    }
                }
            }
